...
export class AuthService {
      ...
      if (user.length > 0) {
        ...
        if (verificar) {
          ...
          const [token, refreshToken] = await Promise.all(
            [
              this.jwtService.signAsync(urserpayload),
              this.jwtService.signAsync(urserpayload, {
  ...
  async refreshToken(token: string): Promise<ResponseAuthDto> {
    try {
      const payload = await this.jwtService.verifyAsync<Record<string, unknown>>(token, { secret: this.config.get(JWT_CONFIG.REFRESH_SECRET) });
      return { token: await this.jwtService.signAsync(payload), refreshToken: token };
    } catch (error: unknown) {
      if (error instanceof UnauthorizedException) {
        throw new UnauthorizedException('Credenciales incorrectas');
      }
